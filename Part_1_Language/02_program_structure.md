...
在某些情况下，`JavaScript`允许你省略语句后的分号。在其他情况下，则必须写上分号，否则下一行会被当作当前语句的一部分。何时可以安全地省略分号的规则稍微复杂且易错。所以，本书中每一条语句都会包含分号。我建议你也这么做，至少在你学习到更多关于省略分号的细微技巧之前。
...
第一个，`var` (“variable”，变量的缩写)，是 2015 版本之前的`JavaScript`定义绑定的方式。在[下一章][link_chapter_3]我们会继续介绍它与 `let` 的不同。目前来说，记住它几乎是在做同样的事情，不过本书中我们很少用它，因为它有一些令人困惑的特性。
...
写一个创建字符串的程序：这个字符串用来表示 8×8 网格，用换行符来分隔各行。在网格的每个位置上，要么是空格，要么是一个 "#" 字符。所有字符应该形成一个国际象棋的棋盘。

把这个字符串传给 `console.log` 应该会显示如下内容：
...
当你写出能产生这个模式的程序时，定义一个绑定 `size = 8`，并修改程序使其对于任意 `size` 都能工作：输出一个指定宽度和高度的网格。
